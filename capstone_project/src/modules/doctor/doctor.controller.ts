...
export class DoctorController {
  ...
  public async addDoctor(@Body() createDoctorDto: CreateDoctorDto, @Headers() headers: any): Promise<boolean> {
    try {
      return await this.doctorService.addDoctor(createDoctorDto, headers);
    }
    catch (error) {
  ...
  public async findAllDoctors(
    ...
    try {
      return await this.doctorService.findAllDoctors(headers, pageWidth, pageSize);
    }
    catch (error) {
      throw new HttpException(`Server response failed! Details: ${error}`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  public async findOneDoctorById(@Param('id') id: string | number, @Headers() headers: Headers): Promise<Doctor> {
    try {
      return await this.doctorService.findOneDoctorById(id,headers);
    }
    catch (error) {
      throw new HttpException(`Server response failed! Details: ${error}`, HttpStatus.INTERNAL_SERVER_ERROR);
    }
